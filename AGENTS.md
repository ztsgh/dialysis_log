...
- 缓存策略：Cache First
- 缓存版本：`dialysis-diary-v14`
- 缓存资源：`index.html`, `css/style.css`, `js/app.js`, `manifest.json`, `icons/icon.svg`
- 动态加载资源：`js/chart.min.js`（统计页首次加载时从网络获取，后续从缓存读取）
...
#### 设置页面班次保存修复 (v1.6.1)
- 问题：设置页面透析班次选择后的保存没有效果
- 根本原因：
  - `app.js` 中缺少 `saveSettings()` 函数
  - 在代码重构时（单一 `app.js` 拆分为模块），该函数被遗漏
- 解决方案：
  - 在 `app.js` 中添加 `saveSettings()` 函数
  - 函数逻辑：从表单读取设置，调用 `Data.saveSettings()` 保存

#### AGENTS.md 文档同步更新 (v1.6.1)
- 更新 Service Worker 缓存版本：`v12` → `v14`
- 更新缓存资源列表：移除 `chart.min.js`（改为动态加载）
- 添加一行添加功能详细说明
- 说明 Chart.js 动态加载机制
